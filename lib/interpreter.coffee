  ...
  defun: (arr, definingEnvironment) ->
    ...
    if typeof(arr[2]) == "function"
      ...
      res = (args, callingEnvironment) ->
        evalled = (xjson arg, callingEnvironment for arg in args)

        arr[2](evalled)

      definingEnvironment[funcName] = res
    else
      # function defined in xjson, create it as a lambda and bind this environment to it.
...
xjson = (arr, env) ->
  ...
  if not isArray arr
    # either a constant or a variable - no string literals in xjson (yet)
